...
/** A little below the bar, so the "current" entry is one you can actually see. */
 ...
 */
export function ReadView({ entries, myRole, cursor, onCursor, onLeaveMode, topInset }: Props) {
  ...
  const scrollToIndex = useCallback((index: number) => {
    ...
    surface.scrollTo({ top: Math.max(0, offsets[clamped] - READING_OFFSET), behavior: 'auto' });
    setCurrent(clamped);
  }, []);
  ...
  const handleScroll = () => {
    ...
    requestAnimationFrame(() => {
      ...
      const surface = surfaceRef.current;
      if (!surface) return;
      const line = surface.scrollTop + READING_OFFSET + 1;
      const offsets = offsetsRef.current;
      let next = 0;
      ...
      <div ref={stackRef} className="readview-stack" style={{ paddingTop: topInset + 12 }}>
        {entries.map((entry, index) => (
          <div
            ...
            className={`entry ${entryTypeClass(entry, myRole)}`}
          >
